...
なお、Basic認証をかける側で`.htpasswd`用のパスワードを作りたい場合は、下記のツールがそのまま使えます。よければご利用ください。意外に`.htpasswd`のファイルってぱっと作りたい時作れないんですよね。。

<RichLink
  url="https://moldspoon.jp/tools/hash"
  icon='ri-lock-password-line'
  title="ハッシュ生成／Basic認証パスワード | Spoon Tools!"
  subtitle="Basic認証の.htpasswdに貼れる形式のパスワードや、各種ハッシュ値を生成できる無料Webツールです。"
  targetBlank={true}
/>
