...
`Wordpress`などでは、アイキャッチ画像がそのままOGPになるケースも多いですが、Next.jsで作る自前のブログなどでは
OGPも自前で実装する必要があります。その場合、半自動的にタイトルが画像になるようなソリューションが求められきますし、
この方法は一度実装すれば手作業は不要になるので、いい解決方法だと思います。
