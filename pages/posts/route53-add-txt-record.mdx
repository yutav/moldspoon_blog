...
なお、追加したTXTレコードが実際に引けるようになったかは、下記のツールでブラウザからそのまま確認できます。
`dig`が使えない環境や、外から見えているかを確かめたいときに便利です。まだ反映待ちなのか設定を間違えているのか、自分の環境から見ているだけだと切り分けづらいんですよね。。

<RichLink
  url="https://moldspoon.jp/tools/dns-lookup"
  icon='ri-global-line'
  title="DNSレコード一括チェッカー | Spoon Tools!"
  subtitle="A・CNAME・MX・TXTなどのレコードをまとめて確認できる無料Webツールです。"
  targetBlank={true}
/>

SPF・DKIM・DMARCのTXTレコードを追加した場合は、書式まで含めて診断できるツールもあります。

<RichLink
  url="https://moldspoon.jp/tools/mail-security"
  icon='ri-mail-check-line'
  title="メールセキュリティ診断（SPF/DKIM/DMARC） | Spoon Tools!"
  subtitle="ドメインを入れるだけで、メール認証用のTXTレコードの設定内容を診断できる無料Webツールです。"
  targetBlank={true}
/>
