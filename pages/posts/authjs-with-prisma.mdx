...
export const meta = {
  title: 'Auth.js(NextAuth)+PrismaでNext.jsにDB認証を導入する手順。sessionでtokenが取れない問題の解決法も',
  date: '2023-11-30T01:33:51.000Z',
  updateDate: '2026-08-26T09:45:00.000Z',
  tags: [
    'Tips',
    '経験者向け',
    'プログラミング',
  ...
  description: 'Auth.js(旧NextAuth)とPrismaアダプターでNext.js App RouterにGoogle認証+DBセッションを導入する手順を、スキーマ定義から実装コード付きで解説します。database戦略でsessionコールバックのtokenがnullになる問題の原因と回避策、Auth.js v5での書き方の変化も追記しました。'
}
...
なお、上記の`secret`に渡す`NEXTAUTH_SECRET`には、推測できない十分な長さのランダム文字列を入れます。
`openssl rand -base64 32`で作るのが定番ですが、手元にコマンドが無いときはブラウザでも生成できます。一度きりしか使わない値なのに、環境が変わるたびに調べ直すことになりがちです。よろしければどうぞ。
...
なお、`token`に値は入っているが中身が期待と違う、というケースもあります。
その切り分けには、JWTをデコードしてペイロードを直接見るのが手早いです。値が空なのか、入っているが想定と違うのかで、疑うべき場所がまるで変わってきます。

<RichLink
  url="https://moldspoon.jp/tools/jwt"
  icon='ri-shield-keyhole-line'
  title="JWTデコーダー | Spoon Tools!"
  subtitle="JWTを貼り付けるだけで、ヘッダー・ペイロード・有効期限を確認できる無料Webツールです。"
  targetBlank={true}
/>
...
### PrismaClientはシングルトンにする

本記事のコードでは説明の簡潔さを優先して`options.ts`で`new PrismaClient()`していますが、
Next.jsの開発サーバはホットリロードのたびにモジュールを再評価するため、<span className="hoso">この書き方だと接続が増殖して
「too many connections」でDBに蹴られる</span>ことがあります。実プロジェクトでは定番のシングルトン化をしておきましょう。

    <CodeBlockTitle title="src/lib/prisma.ts" />
...
### Auth.js v5では設定の書き方が変わる

本記事はNextAuth **v4** ベースですが、v5（パッケージ名も「Auth.js」に統一）では構成が整理されています。

- 設定はプロジェクト直下の`auth.ts`に集約し、`export const { handlers, auth, signIn, signOut } = NextAuth({...})`の形で受け取る
- `route.ts`は`handlers`を再エクスポートするだけになる
- 環境変数は`AUTH_SECRET`・`AUTH_GOOGLE_ID`・`AUTH_GOOGLE_SECRET`の命名にすると**Providerへの受け渡しコードすら不要**になる
- サーバー側でのセッション取得は`getServerSession(options)`ではなく`auth()`を呼ぶだけ

考え方（Prismaアダプター・database戦略・callbacksでの加工）はv4と同じなので、
本記事の内容はv5でも読み替えて使えます。移行時は[公式のマイグレーションガイド](https://authjs.dev/getting-started/migrating-to-v5)を参照してください。
...
DBまわりでは[サーバレスPostgres「Neon」の始め方](https://moldspoon.jp/blog/posts/how-to-start-neon)や
[PrismaのType拡張Tips](https://moldspoon.jp/blog/posts/add-extra-key-to-prisma-type)も書いていますので、あわせてどうぞ。
