...
上記の`secret`に渡す`NEXTAUTH_SECRET`には、推測できない十分な長さのランダム文字列を入れます。
`openssl rand -base64 32`で作るのが定番ですが、手元にコマンドが無いときはブラウザでも生成できます。
...
その切り分けには、JWTをデコードしてペイロードを直接見るのが手早いです。
