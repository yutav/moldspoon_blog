    ...
    <div className="item">
      <div className="flex">
        <Link href={post.url} passHref className="flex-none">
          {isMedium ? (
            <Image src={process.env.baseUrl + "/api/og?v=4&title=" + encodeURI(post.name.length > 40 ? post.name.slice(0, 40) + '...' : post.name)} width="100" height="44" alt={post.name} style={{
              borderRadius: '5px'
            }} className='hover:opacity-80'
            />
          ) : (
            <Image src={process.env.baseUrl + "/api/og?v=3&title=" + encodeURI(post.name.length > 40 ? post.name.slice(0, 40) + '...' : post.name)} width="320" height="140" alt={post.name} style={{
              borderRadius: '12.5px'
            }} className='hover:opacity-80'
            />
          )}
